...
_MLA_STAGE2_HIP_FUSED_MIN_SPLITS = max(
    2, int(os.getenv("VLLM_AITER_MLA_HIP_FUSED_MIN_SPLITS", "32"))
)
...
# ---------------------------------------------------------------------------
# MFMA head-4 kernels for TP=8 Kimi Linear decode (nhead=4, BF16 Q, FP8 KV)
# ---------------------------------------------------------------------------


@triton.jit
def _mla_head4_mfma_qk_logits_kernel(
    q_ptr,
    k_ptr,
    kv_indices_ptr,
    split_ptr_ptr,
    ...
    """Computes QK score tiles for the TP8 local-head4 MLA path.

    Triton's BF16 ``tl.dot`` path on AMD lowers to MFMA-backed matrix ops, so
    this replaces the scalar per-token QK inner product with a tiled matrix
    primitive while leaving the lazy split ABI unchanged.
    """
    ...
    stride_qh_i64 = tl.full((), stride_qh, tl.int64)
    stride_qd_i64 = tl.full((), stride_qd, tl.int64)
    stride_kp_i64 = tl.full((), stride_kp, tl.int64)
    stride_kd_i64 = tl.full((), stride_kd, tl.int64)
    stride_outs_i64 = tl.full((), stride_outs, tl.int64)
    stride_outh_i64 = tl.full((), stride_outh, tl.int64)
    stride_outt_i64 = tl.full((), stride_outt, tl.int64)
    ...
    acc = tl.zeros((NUM_HEADS, BLOCK_N), dtype=tl.float32)
    for k0 in range(0, hidden_dim, BLOCK_K):
        offs_k = k0 + tl.arange(0, BLOCK_K)
        q = tl.load(
            q_ptr
            + offs_h[:, None].to(tl.int64) * stride_qh_i64
            + offs_k[None, :].to(tl.int64) * stride_qd_i64,
            mask=offs_h[:, None] < NUM_HEADS,
            other=0.0,
        ).to(tl.bfloat16)
        k = tl.load(
            k_ptr
            + token_indices[None, :] * stride_kp_i64
            + offs_k[:, None].to(tl.int64) * stride_kd_i64,
    ...
    tl.store(
        out_ptr
        + tl.full((), pid_split, tl.int64) * stride_outs_i64
        + offs_h[:, None].to(tl.int64) * stride_outh_i64
        + offs_n[None, :].to(tl.int64) * stride_outt_i64,
        logits,
        mask=token_mask[None, :],
    )
...
@triton.jit
def _mla_head4_mfma_reduce_kernel(
    scores_ptr,
    k_ptr,
    kv_indices_ptr,
    split_ptr_ptr,
    ...
    """Reduces MFMA-generated score tiles into lazy split-local (N, m, D).

    Fused-head variant: loads V once per tile and reuses it across all
    NUM_HEADS heads (MLA shares the KV cache).
    Grid: (num_kv_splits, ceil(v_head_dim / BLOCK_V)).
    """
    log2e = 1.4426950408889634

    pid_split = tl.program_id(0)
    pid_vblock = tl.program_id(1)

    offs_v = pid_vblock * BLOCK_V + tl.arange(0, BLOCK_V)
    offs_h = tl.arange(0, NUM_HEADS)
    stride_scores_s_i64 = tl.full((), stride_scores_s, tl.int64)
    stride_scores_h_i64 = tl.full((), stride_scores_h, tl.int64)
    stride_scores_t_i64 = tl.full((), stride_scores_t, tl.int64)
    stride_kp_i64 = tl.full((), stride_kp, tl.int64)
    stride_kd_i64 = tl.full((), stride_kd, tl.int64)
    stride_numers_i64 = tl.full((), stride_numers, tl.int64)
    stride_numerh_i64 = tl.full((), stride_numerh, tl.int64)
    stride_numerv_i64 = tl.full((), stride_numerv, tl.int64)
    stride_metas_i64 = tl.full((), stride_metas, tl.int64)
    stride_metah_i64 = tl.full((), stride_metah, tl.int64)
    ...
    run_numer = tl.zeros((NUM_HEADS, BLOCK_V), dtype=tl.float32)

    pid_split_i64 = tl.full((), pid_split, tl.int64)
    score_base = scores_ptr + pid_split_i64 * stride_scores_s_i64

    for n0 in range(0, max_split_tokens, BLOCK_N):
        offs_n = n0 + tl.arange(0, BLOCK_N)
        token_mask = offs_n < split_len

        # All heads' scores in one 2-D load: (NUM_HEADS, BLOCK_N)
        scores = tl.load(
            score_base
            + offs_h[:, None].to(tl.int64) * stride_scores_h_i64
            + offs_n[None, :].to(tl.int64) * stride_scores_t_i64,
            mask=token_mask[None, :],
            other=-float("inf"),
        )
        ...
        # V tile loaded ONCE, shared across all heads: (BLOCK_N, BLOCK_V)
        token_indices = tl.load(
            kv_indices_ptr + split_start + offs_n,
            mask=token_mask,
            other=0,
        ...
        v_tile = tl.load(
            k_ptr
            + token_indices[:, None] * stride_kp_i64
            + offs_v[None, :].to(tl.int64) * stride_kd_i64,
            mask=token_mask[:, None] & mask_v[None, :],
            other=0.0,
        ).to(tl.bfloat16)

        # (H, N) @ (N, V) -> (H, V)
        weighted_v = tl.dot(weights.to(tl.bfloat16), v_tile)
        run_numer = (
    ...
    # Store numerators for all heads: (NUM_HEADS, BLOCK_V)
    tl.store(
        numer_ptr
        + pid_split_i64 * stride_numers_i64
        + offs_h[:, None].to(tl.int64) * stride_numerh_i64
        + offs_v[None, :].to(tl.int64) * stride_numerv_i64,
        run_numer,
        mask=mask_v[None, :],
    )
    if pid_vblock == 0:
        meta_offs = (
            meta_ptr
            + pid_split_i64 * stride_metas_i64
            + offs_h.to(tl.int64) * stride_metah_i64
        )
