...
def _gemm_a16_w16_kernel(
    ...
    if split_k_start < K:
        ...
        a = gl.amd.cdna3.buffer_load(ptr=a_ptr, offsets=a_offs)
        b = gl.amd.cdna3.buffer_load(ptr=b_ptr, offsets=b_offs)
        smem_a.index(1).store(a)
        smem_b.index(1).store(b)
        a_offs += BLOCK_SIZE_K * stride_ak
        b_offs += BLOCK_SIZE_K * stride_bk

        i = 0

        if(gl.amd.cdna3.wave_id() / 4 == 1):
        ...
        for k in range(num_k_iter-2):
            ...
            #gl.amd.cdna3.sched_barrier(0x0)
            #gl.amd.cdna3.iglp_opt(0x1)
            cur_a = smem_a.index(i).load(layout=dot_a_layout)
            cur_b = smem_b.index(i).load(layout=dot_b_layout)

            #gl.amd.cdna3.sched_barrier(0x0)
            gl.amd.cdna3.s_barrier()
            gl.amd.cdna3.s_set_prio(0x0)

            #gl.amd.cdna3.sched_barrier(0x0)
            accumulator = gl.amd.cdna4.mfma(cur_a, cur_b, accumulator)
            smem_a.index(i).store(a)
            smem_b.index(i).store(b)
            #gl.amd.cdna3.sched_barrier(0x0)
