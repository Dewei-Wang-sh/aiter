...
def matmul_kernel(
    ...
    for k in range(num_k_iter):
        with gl.amd.warp_pipeline_stage("stage0"):
            ...
            gl.amd.cdna4.async_copy.buffer_load_to_shared(a_bufs, a_ptr, a_offs)
            gl.amd.cdna4.async_copy.buffer_load_to_shared(b_bufs, b_ptr, b_offs)
        ...
        with gl.amd.warp_pipeline_stage("stage1"):
            ...
            cur_a = a_bufs.load(layout=dot_a_layout)
            cur_b = b_bufs.load(layout=dot_b_layout)
