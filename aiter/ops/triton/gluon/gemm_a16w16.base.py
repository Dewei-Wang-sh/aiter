...
def _gemm_a16_w16_kernel(
    ...
    shared_a: gl.constexpr = gl.SwizzledSharedLayout(
        vec=8, per_phase=4, max_phase=4, order=[1, 0]
    )
    shared_b: gl.constexpr = gl.SwizzledSharedLayout(
        vec=8, per_phase=4, max_phase=4, order=[0, 1]
    )
    ...
    if split_k_start < K:
        ...
        for k in range(num_k_iter):
            ...
            if EVEN_K:
                ...
                a = gl.load(
                    a_ptrs, mask=offs_ak[None, :] < k_span - k * BLOCK_SIZE_K, other=0.0
                )
                b = gl.load(
                    b_ptrs,
                    mask=offs_bk[:, None] < k_span - k * BLOCK_SIZE_K,
                    other=0.0,
            ...
            # accumulator += gl.dot(a, b, input_precision="ieee")
