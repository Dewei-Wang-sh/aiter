...
def test_mla(
    ...
    def test_absorb_decode_triton():
        from aiter.ops.triton.attention.mla_decode_triton import mla_decode_triton
        ...
        (out_result, _), us_triton_decode = run_perftest(
            mla_decode_triton,
            q_nope,
            q_pe,
            kv_c,
            out_triton.view(batch_size, nhead, v_head_dim),
            page_table,
            seq_info,
            sm_scale,
            use_2d_view=use_2d_view,
        ...
        err = checkAllclose(
            out_ref,
            out_triton,
            msg=f"mla_decode-absorb    [golden vs triton_mla]: {us_triton_decode:>8.2f} us......",
        )
